...
class TwoBatchTripletDataLoader(DataLoader):
    ...
    def __iter__(self):
        ...
        self.batches = [data_indices[i:i + self.batch_size] for i in
                        range(0, self.num_samples, self.batch_size)]
    ...
    def _iter_batches(self, data_indices):
        for batch_indices in self.batches:
            ...
            # Directly create batches from indices
            batch_data, batch_concepts, batch_labels = zip(
                *[self.dataset[i] for i in batch_indices])
            rest_data, rest_concepts, rest_labels = zip(
                *[self.dataset[i] for i in rest_indices]) if len(
                rest_indices) > 0 else ([], [], [])

            batch_data = torch.stack(batch_data)
            batch_concepts = torch.stack(batch_concepts)
            batch_labels = torch.stack(batch_labels)

            if rest_data:
                rest_data = torch.stack(rest_data)
                rest_concepts = torch.stack(rest_concepts)
                rest_labels = torch.stack(rest_labels)
            else:
                rest_data = None
                rest_concepts = None
                rest_labels = None

            yield ((batch_data, batch_concepts, batch_labels),
                   (rest_data, rest_concepts, rest_labels))
