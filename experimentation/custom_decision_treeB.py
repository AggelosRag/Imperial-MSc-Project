...
def tree_to_dict(node):
    ...
    if node.left or node.right:
        node_dict[(node.feature_index, node.threshold)] = {
...
def export_tree(tree, feature_names):
    ...
    # Generate colors based on the number of unique classes
    unique_classes = len(tree.tree.num_samples_per_class)
    colors = list(mcolors.CSS4_COLORS.values())[:unique_classes]
    ...
    def add_node(node, node_id):
        if node.left or node.right:
            ...
            node.feature_index, threshold) in tree.fixed_splits:
                threshold_str = "== 0"
            else:
                threshold_str = f"<= {threshold:.2f}"

            dot_data.append(
                f'{node_id} [label="Feature {feature_names[node.feature_index]} {threshold_str}\\n'
                f'gini = {node.gini:.2f}\\nsamples = {node.num_samples}\\n'
                f'value = {list(node.num_samples_per_class.values())}\\nclass = {node.predicted_class}", fillcolor="{colors[node.predicted_class % len(colors)]}"] ;')
            left_id = node_id * 2 + 1
