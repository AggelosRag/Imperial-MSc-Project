...
class CustomDecisionTreeEntropy:
    ...
    class Node:
        def __init__(self, entropy, num_samples, num_samples_per_class,
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        print(f"Node {node_id}: Entropy={self._entropy(y):.4f}, Samples={len(y)}, Class={predicted_class}")
        ...
        for idx in range(n):
            ...
            for i in range(1, m):
                ...
                if thresholds[i] == thresholds[i - 1]:
                    continue
                entropy_left = self._entropy(
                    [num_left[x] for x in range(self.n_classes)])
                entropy_right = self._entropy(
                    [num_right[x] for x in range(self.n_classes)])
                info_gain = best_entropy - (
                            i * entropy_left + (m - i) * entropy_right) / m
                if info_gain > best_info_gain:
                    best_info_gain = info_gain
                    best_idx = idx
                    best_thr = (thresholds[i] + thresholds[i - 1]) / 2
                print(
                    f"Feature {idx}: Threshold {thresholds[i]}, Info gain {info_gain:.4f}")
        print(
            f"Best feature: {best_idx}, Best threshold: {best_thr}, Best info gain: {best_info_gain:.4f}")
    ...
    def _gain_ratio(self, info_gain, y_left, y_right):
        n = len(y_left) + len(y_right)
        split_info = - (len(y_left) / n) * np.log2(len(y_left) / n) - (
                    len(y_right) / n) * np.log2(len(y_right) / n)
        if split_info == 0:
            return 0
    ...
    def _apply(self, inputs):
        node = self.tree
        cumulative_info_gain = 0
        cumulative_gain_ratio = 0
        while node.left:
            ...
            if inputs[node.feature_index] < node.threshold:
                ...
                node = node.right
            left_y = node.num_samples_per_class
            right_y = [node.num_samples - c for c in left_y]
            info_gain = self._information_gain(parent_entropy, left_y, right_y)
            gain_ratio = self._gain_ratio(info_gain, left_y, right_y)
            cumulative_info_gain += info_gain
            cumulative_gain_ratio += gain_ratio
        return node.id, cumulative_info_gain, cumulative_gain_ratio
    ...
    def export_tree(self, feature_names, class_colors, class_names):
        ...
        def add_node(node, node_id):
            if node.left or node.right:
                ...
                dot_data.append(
                    f'{node_id} [label="entropy = {node.entropy:.2f}\\nsamples = {node.num_samples}\\n'
                    f'value = {node.num_samples_per_class}\\nclass = {class_names[node.predicted_class]}", fillcolor="{fillcolor}"] ;')
...
def build_combined_tree(original_tree, leaf_trees, X, y):
    ...
    def copy_tree(node, combined_tree):
        ...
        new_node = CustomDecisionTreeEntropy.Node(
    ...
    combined_tree_obj = CustomDecisionTreeEntropy(n_classes=original_tree.n_classes,
...
def example_usage():
    ...
    clf = CustomDecisionTreeEntropy(n_classes=n_classes, max_depth=1000, min_samples_leaf=300)
    clf.fit(X_train, y_train)
    ...
    for leaf in leaf_nodes:
        ...
        if len(np.unique(y_leaf)) > 1:
            leaf_tree = CustomDecisionTreeEntropy(min_samples_leaf=1,
                                           n_classes=n_classes)
            ...
            leaf_graph.render(f"dt_with_dts_custom_entropy/leaf_tree_{leaf.id}", format='png',
                              cleanup=True)
    ...
    original_graph.render("dt_with_dts_custom_entropy/original_tree", format='png', cleanup=True)
    ...
    combined_graph.render("dt_with_dts_custom_entropy/combined_tree", format='png', cleanup=True)
...
#               [7.3, 6.3],
#               [7.1, 6.0]])
# y = np.array([0, 0, 1, 0, 1, 2, 2, 2])
#
# # Fit the custom decision tree
# tree = CustomDecisionTreeEntropy(n_classes=3, min_samples_split=2, min_samples_leaf=1, max_depth=3)
# tree.fit(X, y)
#
# # Print predictions
...
#
# dot_data = tree.export_tree(feature_names, class_colors, class_names)
# print(dot_data)
